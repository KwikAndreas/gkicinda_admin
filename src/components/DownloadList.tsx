...
const DownloadList: React.FC<DownloadListProps> = ({ refreshTrigger }) => {
          ...
          listData.forEach((file) => {
            if (file.name.endsWith(".pdf")) {
              ...
              const match = file.name.match(
                /WARTA JEMAAT (\d{1,2} \w+ \d{4})\.pdf/i
              ); // Tambah 'i' untuk case-insensitivity jika perlu fleksibilitas
              let fileDate: Date | null = null;
              if (match && match[1]) {
                // Pastikan format parsing sesuai dengan format yang digunakan saat mengunggah
                const parsedDate = parse(match[1], "dd MMMM yyyy", new Date(), {
                  locale: id,
                });
  ...
  return (
    <div className="border border-gray-300 p-4 rounded-lg overflow-x-auto bg-white">
      <h3 className="font-semibold mb-2">File PDF Tersedia</h3>
      {pdfFiles.length === 0 ? (
        <p className="text-gray-500">Tidak ada file PDF yang ditemukan.</p>
      ) : (
